...
MODEL_PATH = Path(os.getenv("MODEL_PATH", BASE_DIR / "final_mobilenetv2_breast_cancer.keras"))
IMG_SIZE = (224, 224)
MAX_IMAGE_BYTES = 10 * 1024 * 1024

BACK4APP_APP_ID = os.getenv("BACK4APP_APP_ID", "").strip()
BACK4APP_REST_API_KEY = os.getenv("BACK4APP_REST_API_KEY", "").strip()
BACK4APP_SERVER_URL = os.getenv("BACK4APP_SERVER_URL", "https://parseapi.back4app.com").rstrip("/")
...
def back4app_headers():
    return {"X-Parse-Application-Id": BACK4APP_APP_ID,
            "X-Parse-REST-API-Key": BACK4APP_REST_API_KEY,
            "Content-Type": "application/json"}


def save_prediction(record: dict):
    response = requests.post(f"{BACK4APP_SERVER_URL}/classes/{BACK4APP_CLASS}",
                             json=record, headers=back4app_headers(), timeout=15)
    response.raise_for_status()
    return response.json().get("objectId")


def fetch_history(user_id: str, limit: int = 50):
    params = {"where": json.dumps({"userId": user_id}), "order": "-createdAt",
              "limit": max(1, min(limit, 100))}
    response = requests.get(f"{BACK4APP_SERVER_URL}/classes/{BACK4APP_CLASS}",
                            params=params, headers=back4app_headers(), timeout=15)
    response.raise_for_status()
...
app = FastAPI(title="Breast Cancer Detection API", version="1.0.0", lifespan=lifespan)
...
@app.get("/")
def root():
    return {"message": "Breast Cancer Detection API is running", "docs": "/docs"}
...
@app.post("/predict")
async def predict(file: UploadFile = File(...), user_id: str | None = Query(default=None)):
    ...
    output = await run_in_threadpool(app.state.model.predict, processed, verbose=0)
    raw_score = float(output[0][0])
    label, confidence = ("malignant", raw_score) if raw_score > 0.5 else ("benign", 1.0 - raw_score)
    result = {"success": True, "filename": file.filename, "prediction": label,
              "confidence": round(confidence, 4), "raw_score": round(raw_score, 4),
    ...
    if user_id and back4app_configured():
        record = {"userId": user_id, "filename": (file.filename or "uploaded_image")[:200],
        ...
        try:
            ...
            result["history_warning"] = "Prediction completed, but history could not be saved."
    else:
        result["history_saved"] = False
        if user_id:
            result["history_warning"] = "Back4App credentials are not configured."
    return result


@app.get("/history")
async def history(user_id: str = Query(..., min_length=1), limit: int = Query(50, ge=1, le=100)):
    if not back4app_configured():
        raise HTTPException(status_code=503, detail="Back4App is not configured.")
    try:
        records = await run_in_threadpool(fetch_history, user_id, limit)
        return {"success": True, "count": len(records), "results": records}
    except requests.RequestException:
        raise HTTPException(status_code=502, detail="Unable to retrieve history from Back4App.")
